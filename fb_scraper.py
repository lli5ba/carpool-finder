...
debug = False
annotation_list_binary = ["rider or driver (0, 1)", "spam or not spam (0, 1)", "one-way or roundtrip(0, 1)"]
annotation_list_str = ["departure_time_informal_1", "departure_location_informal_1",
                       "destination_location_informal_1", "departure_time_informal_2",
                       "departure_location_informal_2", "destination_location_informal_2",
                       "departure_time_formal_1", "departure_location_formal_1",
                       "destination_location_formal_1", "departure_time_formal_2",
                       "departure_location_formal_2", "destination_location_formal_2"]
...
def getFacebookGroupData(group_id, access_token):
    base = "https://graph.facebook.com/v2.8"
    node = "/%s" % group_id
...
def processFacebookPageFeedStatus(status, access_token):
    ...
    if 'message' in status.keys():
        ...
        if debug:
            print 'price: ' + str(status_price)
            print 'location: ' + str(status_location)
    ...
    return (status_id, status_permalinkurl, status_published,
            ...
            link_name, num_reactions, num_comments,
...
def scrapeFacebookPageFeedStatus(group_id, access_token):
    with open('%s_facebook_statuses.csv' % group_id, 'wb') as file:
        w = csv.writer(file)
        w.writerow(["status_id", "status_permalinkurl", "status_published",
            "status_message", "status_location", "status_price",
            "status_author", "status_type", "status_link",
            "link_name", "num_reactions", "num_comments",
...
def scrapeMultipleFacebookPageFeedStatus(group_id_list, access_token):
    with open('group3_facebook_statuses.csv', 'wb') as file:
        w = csv.writer(file)
        w.writerow(["group_name", "group_id", "status_id", "status_permalinkurl", "status_published",
            "status_message", "status_location", "status_price",
            "status_author", "status_type", "status_link",
            "link_name", "num_reactions", "num_comments",
            "num_shares", "num_likes", "num_loves", "num_wows",
            "num_hahas", "num_sads", "num_angrys"] + annotation_list_binary + annotation_list_str)
        total_num_processed = 0 # keep a count on how many we've processed total
        scrape_starttime = datetime.datetime.now()
        for group_id in group_id_list:
            num_processed = 0   # keep a count on how many we've processed per group
            has_next_page = True
            print "Scraping %s Facebook Page: %s\n" % \
                  (group_id, scrape_starttime)

            group_name = getFacebookGroupData(group_id, access_token)
            statuses = getFacebookPageFeedData(group_id, access_token, 100)
            ...
            while has_next_page:
                for status in statuses['data']:
                    # Ensure it is a status with the expected metadata
                    if 'message' in status:
                        #append group_id to tuple
                        w.writerow((group_name, group_id ) + processFacebookPageFeedStatus(status, \
                                                                access_token) +
                                   (-1, )*len(annotation_list_binary) +
                                   ('', )*len(annotation_list_str))
                    ...
                    if num_processed % 100 == 0:
                        print "%s Statuses Processed, %s Total: %s" % (num_processed,
                                total_num_processed, datetime.datetime.now())
        ...
        print "\nDone!\n%s Statuses Processed in %s" % \
                (total_num_processed, datetime.datetime.now() - scrape_starttime)

if __name__ == '__main__':
    #scrapeFacebookPageFeedStatus(group_id, access_token)
    text_file = open(file_name, "r")
    group_id_list = filter(None, [s.strip() for s in text_file.read().splitlines()])
    scrapeMultipleFacebookPageFeedStatus(group_id_list, access_token)
